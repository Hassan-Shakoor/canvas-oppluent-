...
function PartnerRow({partner,id, checked, handleSelect}) {
  ...
  return (
      ...
      <div className="rt-tr -odd" role="row">
        ...
        >
          <label className="checkbox" htmlFor="select_partner_278120">
            <input className="checkbox__input" type="checkbox" checked={checked.includes(id)}/>
        ...
        <div
          ...
          <div className="ReactTable__avatar-container">
            {partner
              ?.profilePhoto === ""
                ? <div
                    ...
                    <p className="ReactTable__avatar_initials">{`${ (partner
                          ?.firstName)
                        .slice(0, 1)
                        .toUpperCase()} ${ (partner
                          ?.lastName)
            ...
            <div className="ReactTable__info-lines">
              <h5 className="ReactTable__top-line">{`${partner
                  ?.firstName} ${partner
                    ?.lastName}`}</h5>
              <p className="ReactTable__sub-line">{partner
                  ?.email}</p>
            </div>
          </div>
        </div>
        <div
          ...
          {partner
            ?.primary
              ? (
                <span className="btn btn_badge">Primary</span>
              )
              : (
                <button type="button" className="btn btn_gray btn_no-text-transform me-2">
        ...
        <div
          ...
          maxWidth: 40
            ...
            <Dropdown
                trigger={['click']}
                overlay={<PartnersDropdownMenu partnerId={id} setConfirmDelete={setConfirmDelete}/>}
            >      
    ...
    <ConfirmationModal
        ...
        submit={(event) => handleDelete(event,id)}
    />
    }
    </>
  )
